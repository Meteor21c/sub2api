    ...
    <DataTable :columns="columns" :data="orders" :loading="loading">
      ...
      <template #cell-pay_amount="{ value, row }">
        <div class="text-sm">
          ...
          <span v-else-if="row.fee_rate < 0" class="ml-1 text-xs text-green-600 dark:text-green-400" :title="t('payment.orders.discount') + ': ' + Math.abs(row.fee_rate) + '%'">
            ({{ t('payment.orders.discount') }} {{ Math.abs(row.fee_rate) }}%)
          </span>

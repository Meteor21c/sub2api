export default {
    accounts: {
      ...
      loadFactorHint: 'Used as the denominator for load rate: load is approximately (active concurrency + queued requests) / load factor. A higher value is less likely to be considered full and is scheduled more often; it does not change the concurrency cap. Blank uses concurrency.',
      priority: 'Priority',
      priorityHint: 'In the regular scheduler, lower values are preferred first; load rate and last-used time break ties. It affects account choice, not the concurrency cap.',

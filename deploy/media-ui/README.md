...
- 豆包 Seedance 先用现有每秒配置预扣余额；任务完成后按盈合实际输出 Token、创建时密钥专属场景价再加价 15% 结算，多退少补，失败退款。页面经认证的 `/v1/videos/pricing` 显示官方价、上游折后价、本站价和预扣额，任务创建响应会固定实际价格快照。本站按用户指定的人民币／美元 1:1 数值口径换算。
- Kling 与 Grok 保持原有按秒计费；页面对其超过 15 秒或 4K 的请求不显示精确报价。

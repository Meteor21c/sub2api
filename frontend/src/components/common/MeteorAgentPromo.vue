<template>
  <a
    ...
    :title="props.collapsed ? labelText : undefined"
  >
    <span
    ...
    <span v-else class="flex min-w-0 items-center gap-3">
      <span
        ...
        :class="props.variant === 'header' ? 'h-7 w-7 rounded-lg' : 'h-10 w-10'"
      >
        <Icon name="download" :size="props.variant === 'header' ? 'sm' : 'md'" />
      </span>
      <span class="min-w-0 flex-1">
        <span
          class="block truncate font-semibold text-gray-900 dark:text-white"
          :class="props.variant === 'header' ? 'text-xs' : 'text-sm'"
        >
          {{ labelText }}
        </span>
        <span
...
const labelText = computed(() =>
  props.variant === 'header' || (props.variant === 'sidebar' && props.collapsed)

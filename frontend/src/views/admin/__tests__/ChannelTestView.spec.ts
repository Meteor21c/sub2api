...
describe('Availability V2 administrator page', () => {
  ...
  it('creates an automatic durable conversation and renders every routed attempt', async () => {
    ...
    const conversationPanel = wrapper.get('[data-testid="availability-conversation-panel"]')
    expect(conversationPanel.find('[data-testid="availability-events"]').exists()).toBe(false)
    expect(conversationPanel.find('[data-testid="availability-attempts-panel"]').exists()).toBe(false)
